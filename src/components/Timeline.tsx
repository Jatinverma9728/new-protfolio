...
const TimelineItem: React.FC<TimelineItemProps> = ({ title, company, date, position }) => {
  ...
  return (
    <div ref={ref} className={`flex items-center ${position === 'left' ? 'flex-row-reverse' : 'flex-row'} w-full mb-8`}>
      <div className={`w-5/12 ${inView ? (position === 'left' ? 'animate-slide-left' : 'animate-slide-right') : 'opacity-0'}`}>
...
export const Timeline = () => {
  ...
  return (
    <div ref={timelineRef} className="relative max-w-3xl mx-auto" onMouseMove={handleMouseMove}>
      {/* Timeline line container */}
      <div className="absolute left-1/2 transform -translate-x-1/2" style={{ top: '24px', bottom: '24px' }}>
        {/* Base line */}
        <div 
          ref={lineRef}
        ...
        <div 
          ...
          style={{
            background: 'linear-gradient(to bottom, transparent, #60a5fa, #3b82f6, #60a5fa, transparent)',
            filter: 'blur(3px)',
            opacity: 1,
            width: '5px',
            ...
            <div className="flex items-center">
              {exp.position === 'left' ? (
                <div className="w-1/2 pr-8 text-right">
                  <div className="timeline-card relative overflow-hidden bg-gray-900/50 backdrop-blur-sm 
