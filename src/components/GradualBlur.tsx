...
type GradualBlurProps = PropsWithChildren<{
  ...
  tabletWidth?: string;
  desktopWidth?: string;
  sticky?: boolean; // New prop to make the blur stick to parent bottom
  preset?:
    | 'top'
    | 'bottom'
    | 'left'
...
const GradualBlur: React.FC<GradualBlurProps> = props => {
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    const baseStyle: CSSProperties = {
      position: isPageTarget ? 'fixed' : (config.sticky ? 'sticky' : 'absolute'),
      pointerEvents: config.hoverIntensity ? 'auto' : 'none',
      opacity: isVisible ? 1 : 0,
      transition: config.animated ? `opacity ${config.duration} ${config.easing}` : undefined,
      zIndex: isPageTarget ? config.zIndex + 100 : config.zIndex,
      ...config.style,
      // For sticky positioning to work, we need to ensure the parent has proper positioning
      // and the sticky element has a position value
      ...(config.sticky && !isPageTarget && {
        position: 'sticky',
